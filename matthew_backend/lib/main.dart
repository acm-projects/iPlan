...
void main() async {
  ...
  if (contactPermission!.isDenied) {
    print(
        "Contact permissions are required to use the collaboration feature. \nPlease allow contact permissions.");
  ...
  if (contactPermission!.isDenied) {
    print(
        "In order to use the Collaborate feature of this app, please enable contact permissions");
    openAppSettings();
    ...
    for (Contact curr in contacts) {
      print(
          "${curr.displayName}${curr.company == null ? "" : " (${curr.company})"}:\n");
      ...
      if (addresses!.isNotEmpty) {
        ...
        for (int i = 0; i < addresses.length; i++) {
          print(
              "\t\t-> ${addresses[i].toString().trim().replaceAll("\n", " ")}\n");
        }
      }
